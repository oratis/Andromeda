...
- **`andromeda hardware check` is not a trust gate.** A forged manifest reaches `certified` without `--artifact-root`, because HCM has no manifest-level signature yet.
